-- START /lua/autorun/sh_rpg_chat.lua
if SERVER then AddCSLuaFile() end

RPG_CHAT = RPG_CHAT or {}
RPG_CHAT.Version = "0.1.1"

-- Channel types
RPG_CHAT.TYPE = { GLOBAL = 1, WHISPER = 2, ADMINS = 3, SYSTEM = 4 }

-- Net names are declared here for reference only; registered on server in rpg_chat_sv.lua
RPG_CHAT.NETS = {
  "rpg_chat_send",
  "rpg_chat_broadcast",
...
-- Colors (client-only use, but defined shared for consistency)
RPG_CHAT.Colors = {
...
-- Emoji alias map (extend as needed)
RPG_CHAT.Emoji = {
  [":shrug:"]   = "¯\\_(ツ)_/¯",
  [":heart:"]   = "❤️",
}
...
function RPG_CHAT.ParseEmoji(text)
  for k,v in pairs(RPG_CHAT.Emoji) do text = string.Replace(text, k, v) end
  return text
end
...
-- Prefix builder (server caches top rank name)
...
-- Client recent buffer (declared for both realms to reference symbol)
if CLIENT then
  RPG_CHAT.RecentBuffer = RPG_CHAT.RecentBuffer or {} -- { {ts, type, sender64, senderName, text, target64?}, ... }
end
-- END /lua/autorun/sh_rpg_chat.lua
